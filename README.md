...
Workforce internship programs track placement starts well. What they rarely track is what happens after: which students exit early, which employer partnerships are expensive relative to what they produce, how dependent the pipeline is on a handful of partnerships, whether students who "complete" are going the full distance or stopping early, whether placement volume is trending or just noisy, and what it actually costs per fiscal year to fully fund the program as it runs.

This analysis answers seven questions the program's standard reporting could not:
...
└── figures/
    ...
    ├── fig2_exit_timing.png
    ├── fig3_outcome_by_campus.png
    ├── fig3b_full_term_completion.png
    ├── fig3c_full_term_by_employer.png
    ├── fig4_employer_quality.png
    ├── fig5_cost_per_completion.png
    ├── fig6_term_volume_trend.png
    ├── fig7_top_roles.png
    ├── fig8_role_concentration.png
    ├── fig9_policy_recommendations.png
    └── fig10_fiscal_year_budget.png
```
...
Applies a five-category outcome framework derived from operational knowledge: Planned Completion, Re-engagement, Student Exit, Employer Exit, Administrative Exit. Structured fixed-length placements (~6-week programs) are flagged separately so they don't distort attrition timing.

**Part 1B — Full-Term Completion Rate (9-Month Benchmark)**
Of students who reach Planned Completion, how many actually go the full 9-month placement length versus completing earlier? Breaks completion tenure into monthly bins and identifies which employers retain interns the longest.

**Part 2 — Employer Quality Analysis**
Weighted scoring model (50% completion, 30% retention, 20% re-engagement). Identifies at-risk partners exceeding a 30% student exit rate at actionable volume.

**Part 3 — Cost Per Completed Placement**
Total wages paid ($18/hr × hours) divided by completions, per employer. Reframes employer quality in dollar terms — the metric a budget owner or grant officer actually needs, and one that completion rate alone can hide.

**Part 4 — Term-to-Term Volume Trend**
Placement volume by term (Spring/Summer/Fall, 2022–present), with a 3-term rolling average and seasonal averaging to separate trend from noise.

**Part 5 — Role Concentration Risk**
Top 7 roles and top 10 employer-role pairings by volume, plus a Herfindahl-Hirschman-style concentration index to quantify pipeline dependency risk in a single number.

**Part 6 — Early Exit Prediction Model**
Binary classifier (Logistic Regression, Random Forest, Gradient Boosting compared via 5-fold stratified CV) flagging placements at elevated risk of student exit before day 60.

**Part 6B — Fiscal Year Budget Requirement**
Retroactive calculation of what the program's budget would have needed to be each fiscal year (Sept 1 – Aug 31) to fully fund every placement actually attempted, including placements that ended early. Wages are allocated proportionally by calendar day across fiscal year boundaries and reconcile exactly to the source data. Coordinator and admin/misc overhead are included as clearly labeled placeholder estimates.

**Part 7 — Policy Recommendations**
Six recommendations, each tied directly to a specific finding above, prioritized HIGH / MEDIUM / LOW with concrete operational actions.
