...
Built from operational experience managing a workforce development program across five campuses. Every analytical decision in this project, including the outcome classification framework, the structured-program handling, the full-term completion benchmark, the cost modeling, the fiscal-year budget reconstruction, and the deep-dive analyses below, reflects domain knowledge about how the program actually worked, not assumptions from the outside.
...
| Placement length | Up to 9 months / 270 days (structured employer programs run ~6 weeks). $14,040 is the hard physical maximum any single placement can pay (9 months x 20 hrs/week x $18/hr), enforced exactly in the dataset. |
...
| Cheapest completion (cost per placement, by employer) | $2,177 (Quantum Institute Fellows, Inc) |
| Most expensive completion (cost per placement, by employer) | $17,461 (Millio's Youth and Outreach Services) |
| Program-wide blended cost per completion | $8,591 |
...
| **5-year average fiscal-year budget required** | **$1,509,809** (intern wages: $1,050,749 + 5-person coordinator team at $75,000 each + admin overhead) |
...
**A note on the fiscal-year budget figures:** the intern wage allocation is exact. Every dollar in the dataset's `Total Pay` column is accounted for, and the calculation reconciles to the cent. Every individual placement's wages are also hard-capped at $14,040, the true physical maximum one intern can earn under the program's own rules (9 months x 20 hrs/week x $18/hr); no placement in the dataset exceeds this. Coordinator overhead ($375,000/year, reflecting a 5-person coordinator team at $75,000 each) and admin/misc overhead (8% of wage spend) are explicitly labeled in the notebook. The coordinator figure reflects a real stated team structure, while the 8% admin/misc rate remains a placeholder estimate, not a measured figure.
...
All student identifiers are anonymized. Campus names are anonymized. The institution is not identified. Employer names identifying the specific county or city have been generalized. The dataset complies with FERPA requirements, and no individual student information is recoverable from the published files.
