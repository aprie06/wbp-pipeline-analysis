...
def tenure_for_outcome(outcome, emp_type):
    ...
    if outcome == 'Planned Completion':
        ...
        # bucket. The final bucket (270-285) is deliberately set so that
        # landing in it reliably produces a "reached full term" completion
        # (tenure >= 270 days) -- this is a TIMING distribution choice.
        #
        # IMPORTANT: tenure_days alone does not determine pay. The actual
        # dollar ceiling ($18/hr x 20hr/wk x 270 days = $14,040.00) is
        # enforced separately and unconditionally below via HARD_MAX_PAY,
        # regardless of how many tenure days a placement is assigned. A
        # placement's recorded END DATE can land slightly past the 270-day
        # mark due to ordinary administrative processing delays (the official
        # close-out date isn't always the literal last paid day), but wages
        # are always capped at the true 9-month-paid maximum.
        month_buckets = [
...
def generate_dataset(n_total=1223):
    ...
    for _ in range(n_total):
        ...
        # Hard safeguard: no placement can ever pay more than the true
        # physical maximum (9 months x 20 hrs/wk x $18/hr = $14,040.00),
        # regardless of how tenure or hours_per_week were generated.
        HARD_MAX_PAY = round((MAX_DURATION_DAYS / 7) * MAX_HOURS_WEEK * PAY_RATE, 2)
        if total_pay > HARD_MAX_PAY:
            total_pay = HARD_MAX_PAY
            total_hours = round(HARD_MAX_PAY / PAY_RATE, 1)
